...
```python
from UI401 import Ui_MainWindow
...
class MainWindow(QMainWindow):  # 继承主窗口函数的类, 继承编写的主函数
    def __init__(self, parent=None):
        ...
        self.ui.pushButton_3.clicked.connect(self.A)
    ...
    def on_pushButton_3_clicked(self):
        ...
        self.DB.setDatabaseName('MES1.db')  # 设置数据库名称
        if self.DB.open():  # 打开数据库
            print('数据库打开成功')
            ## 打开数据表
            self.tabModel = QSqlTableModel(self, self.DB)  # 创建数据表的模型
            self.tabModel.setTable('工件信息表')  # 设置需要连接的数据表
            self.tabModel.setEditStrategy(QSqlTableModel.OnManualSubmit)
            self.ui.enter_tableView.setModel(self.tabModel)  # 为一个QTableView组件设置一个QSqlTabelModel模型
        ...
        ##获取字段名和序号的字典数据
        empty=self.tabModel.record()  #得到的是一个空的记录，获取表的字段定义
        print(self.tabModel.rowCount())  #打印行数
        self.fldNum={}
        ...
        print(self.fldNum)
        print(self.tabModel.record().count())
        print(self.tabModel.record(2))
        ...
        self.mapper.addMapping(self.ui.lineEdit,self.fldNum['种类'])
        self.mapper.toFirst()  # 移动到首记录

        #选中数据时信号的发射
        self.selModel=QItemSelectionModel(self.tabModel)
        #self.selModel.currentChanged.connect(self.do_currentRowChanged())
        self.selModel.currentRowChanged.connect(self.do_currentRowChanged)
        #self.ui.enter_tableView.setModel(self.tabModel)  # 设置数据模型
        self.ui.enter_tableView.setSelectionModel(self.selModel)  # 设置选择模型
    ...
    def on_addButton_clicked(self):
        ...
        query.prepare('''INSERT INTO 工件信息表 (RFID,种类,货位,生产商,
                      批次,入库时间,重量) VALUES(:RFID,:kind, :space,:manufacture,:batch,
                      :enterDate,:weight)''')
        # INSER INTO 工件信息表(RFID, 种类, 货位, 生产商,
        #       批次, 入库时间, 重量)
        # VALUES('A000001', '齿轮', 'A03', 'XJTU', 2, '2020/4/23', 20)
        x=400
        for i in range(5):
            x=x+1
            query.bindValue(":RFID", str(x+1))
            query.bindValue(":kind", '齿轮')
            query.bindValue(":space", 'A02')
            query.bindValue(":manufacture", '西交一厂')
            query.bindValue(":batch", 2)
            query.bindValue(":enterDate", '2020/4/23')
            query.bindValue(":weight", 10);
...
```python
query = QSqlQuery(self.DB)
query.exec("SELECT RFID,种类,生产商 FROM 工件信息表 ")
rec=query.record()
print(rec.count())
print(rec.value('RFID'))
```
...
class MainWindow(QMainWindow):  # 继承主窗口函数的类, 继承编写的主函数
    ...
    def open_alltable(self):
        ...
        self.ui.current_tableView.setModel(self.current_tabModel)  # 为一个QTableView组件设置一个QSqlTabelModel模型
        ...
        #print(self.current_tabModel.rowCount())  #打印行数
        self.fldNum={}
        for i in range(empty.count()):
            filedname=empty.fieldName(i)
            empty.field(filedname).setReadOnly(True)  #每个字段设置为只读
...
#################################现有库存操作函数####################################################
    ...
    #     check=True
    #     for i in range(10):
    #         query.bindValue(":RFID", str(x+i))
    #         query.bindValue(":kind", '直齿轮')
    #         query.bindValue(":space", 'A02')
    #         query.bindValue(":manufacture", '西交一厂')
    #         query.bindValue(":batch", 2)
    #         query.bindValue(":enterDate", '2020/4/23')
    ...
    def on_add_pushButton_clicked(self):
        print('已经点击')
        self.qryModel=QSqlQueryModel(self)
        self.qryModel.setQuery("SELECT RFID,种类 FROM 工件信息表 ")
        print(self.qryModel.rowCount())
        curRec=self.qryModel.record(2)  #返回第2+1条记录
        print(curRec.value('RFID'))  #返回当前记录中名为RFID的字段的值
        print(self.qryModel.record().field('种类').value())
    ...
    def on_delete_pushButton_pressed(self):
        query= QSqlQuery(self.DB)
        print('开始删除')
        check=query.exec("DELETE FROM 工件信息表 WHERE 批次=2")
        # for i in range(3):
        #     query.bindValue(":ID",str(802+i))
        #     res=query.exec()
        #     check=check&res
        if check==False:
            QMessageBox.critical(self, "错误",
...
```python
 @pyqtSlot()
    def on_add_pushButton_clicked(self):
        self.qryModel=QSqlQueryModel(self)
        self.qryModel.setQuery("SELECT RFID,种类,生产商,批次,入库时间 FROM 工件信息表 ")
        check = True
        query = QSqlQuery(self.DB)
        # query.prepare('''INSERT INTO 工件信息表 (RFID,种类,货位,生产商,
        #               批次,入库时间,重量) VALUES(:RFID,:kind, :space,:manufacture,:batch,
        #               :enterDate,:weight)''')
        query.prepare('''INSERT INTO 入库记录表 (RFID,种类,生产商,
                      批次,入库时间) VALUES(:RFID,:kind, :manufacture,:batch,
                      :enterDate)''')
        for i in range(self.qryModel.rowCount()):
            print(i)
            curRec=self.qryModel.record(i)
            print(curRec.value('种类'))
            query.bindValue(":RFID", curRec.value('RFID'))
            query.bindValue(":kind", curRec.value('种类'))
            #query.bindValue(":space", 'A02')
            query.bindValue(":manufacture", curRec.value('生产商'))
            query.bindValue(":batch", curRec.value('批次'))
            query.bindValue(":enterDate", curRec.value('入库时间'))
            #query.bindValue(":weight", 10);
            res=query.exec()  # 执行SQL语句
            check=check&res
...
```python
    ...
    def on_enterList_pushButton_clicked(self):
        date=self.ui.dealdate.date().toString(Qt.ISODate)
        print(date)
        print(type(date))
        query = QSqlQuery(self.DB)
        query.prepare("UPDATE 工件信息表 SET 入库时间=:date")
...
class MainWindow(QMainWindow):  # 继承主窗口函数的类, 继承编写的主函数
    def __init__(self, parent=None):
        ...
        self.enterList={}
        self.outList={}
        self.currentList = self.statistics(self.current_tabModel)  # 获取入库记录里各种工件的数量
...
#################################主操作界面####################################################
    ...
    def on_enterScan_pushButton_clicked(self):
        ...
        n=self.enter_tabModel.rowCount()   #库存的已记录数
        name_RFID={'直齿轮':'01B0E00000020010B1','斜齿轮':'01B0E00000030010B2',
                   '齿轮轴':'01A0E00000010010A1','光轴':'01A0E00000010010A2',
                   '深沟球轴承':'01C0E00000020010C1','调心球轴承':'01C0E00000010010C2'}
        ...
        for key in self.enterList.keys():   #对要入库的工件种类进行迭代
            for i in range(self.enterList[key]):
                no=enteredList[key]+i+1
                RFID=name_RFID[key]+str(no).zfill(6)
        ...
        currentQuery.prepare('''INSERT INTO 工件信息表 (RFID,种类,货位,生产商,
                      批次,入库时间,重量) VALUES(:RFID,:kind,:location,:manufacture,:batch,
                      :enterDate,:weight)''')
        enterQuery.prepare('''INSERT INTO 入库记录表 (RFID,种类,生产商,
                      批次,入库时间) VALUES(:RFID,:kind, :manufacture,:batch,
                      :enterDate)''')
        #准备数据
        RFID_kind=dict([(value, key) for (key, value) in name_RFID.items()])
        RFID_manufacture={'E0000001':'西安交大一厂','E0000002':'西安交大二厂','E0000003':'西安交大三厂'}
        max_batch = self.enter_tabModel.record(n - 1).value('批次')  # 批次按顺序排列，查找最大批次
        date = self.ui.dealdate.date().toString(Qt.ISODate)
        weightList={'直齿轮':15,'斜齿轮':20,'齿轮轴':7,'光轴':5,'深沟球轴承':2,'调心球轴承':1}
        check=True
        #扫码并写入记录表
        with open(dbFilename,'r',encoding='utf8') as f:
            ...
            while s!='':
                ...
                enterQuery.bindValue(":enterDate", date)
    ...
    def warning(self):
        ...
        if num>self.currentList[outKind]:
    ...
    def on_outScan_pushButton_clicked(self):
        currenRow = self.current_tabModel.rowCount()  # 库存的已记录数
        outRow=self.out_tabModel.rowCount()
        RFID_list=[]
        #从现有库存里取符合条件的RFID
        for i in range(currenRow):
            curRec=self.current_tabModel.record(i)
            kindname=curRec.value('种类')
            if kindname in self.outList and self.outList[kindname]>0:
                RFID_list.append(curRec.value('RFID'))
                self.outList[kindname]-=1
        #将RFID写入模拟货物
        ...
        outQuery.prepare('''INSERT INTO 出库记录表 (RFID,种类,生产商,
                              批次,出库时间) VALUES(:RFID,:kind, :manufacture,:batch,
                              :outDate)''')
        currentQuery.prepare('''DELETE FROM 工件信息表 WHERE RFID=:ID''')
        #获取出库记录表里的最大批次
        if self.out_tabModel.record(0).isNull('批次')==True:
            batch=0
        else:
            batch=self.out_tabModel.record(outRow-1).value('批次')
...
#################################入库记录操作函数####################################################
    ...
    def enter_filter(self) :
        ...
        if self.ui.enterCheckBox.isChecked()==True:
            if kind=='全选':
                self.enter_tabModel.setFilter("入库时间='%s'" % (date))
            else:
                self.enter_tabModel.setFilter("入库时间='%s' and 种类='%s'" % (date,kind))
        else:
            if kind=='全选':
    ...
    def on_delete_pushButton_pressed(self):
        query= QSqlQuery(self.DB)
        print('开始删除')
        #query.exec("DELETE FROM 工件信息表 WHERE 批次>=2")
        check=query.exec("DELETE FROM 出库记录表 WHERE 批次=2")
       # check = query.exec("DELETE FROM 入库记录表 WHERE 批次>=2")
        if check==False:
            QMessageBox.critical(self, "错误",
                                 "删除记录出现错误\n" + query.lastError().text())
        else:
...
from PyQt5.QtCore import (pyqtSlot, Qt, QItemSelectionModel,
                          QModelIndex, QFile, QIODevice, QDate)
from PyQt5.QtSql import QSqlDatabase, QSqlTableModel, QSqlRecord, QSqlQuery, QSqlQueryModel
from Plotly_Draw import DemoWindow
...
class MainWindow(QMainWindow):  # 继承主窗口函数的类, 继承编写的主函数
    def __init__(self, parent=None):
        ...
        self.enterList = {}
        self.outList = {}
        self.capacity={'直齿轮': 18, '斜齿轮': 18, '齿轮轴': 24, '光轴': 24, '深沟球轴承': 36, '调心球轴承': 36}
        self.currentList = self.statistics(self.current_tabModel)  # 获取入库记录里各种工件的数量
        print(self.currentList)
    ...
    def enter_Warning(self):
        ...
        enterKind = self.ui.enterKind_comboBox.currentText()
        if num + self.currentList[enterKind]>self.capacity[enterKind]:
            self.ui.enterList_pushButton.setEnabled(False)
            # self.ui.enterScan_pushButton.setEnabled(False)
            warningText = '⚠警告！' + enterKind + '已经超出库存上限！'
        else:
...
#################################现有库存界面####################################################
    ...
    def current_filter(self):
        ...
        if kind == '全选':
            ...
            print(self.currentList[kind])
            progress = int((100 * self.currentList[kind] / self.capacity[kind]) + 0.5)
...
#################################出库记录界面####################################################
    ...
    def out_filter(self):
        ...
        if self.ui.outCheckBox.isChecked() == True:
            if kind == '全选':
                self.out_tabModel.setFilter("出库时间='%s'" % (date))
            else:
                self.out_tabModel.setFilter("出库时间='%s' and 种类='%s'" % (date, kind))
        else:
            if kind == '全选':
...
#################################数据可视化################################################
    ...
    def recordDemo(self,model):
        ...
        dateName = model.record().fieldName(4)  # 入库时间&出库时间
        for i in range(enterRow):
            ...
            if kind not in record:  # 工件种类第一次检索到，将其添加到reord中
                dateRecord[date] = 1  # 同时将对应的时间添加到record中
                record[kind] = dateRecord
            else:
                if date not in record[kind]:  # 种类存在，但是第一次检索到某个日期时
                    record[kind][date] = 1
                else:
                    record[kind][date] += 1
        print(record)
...
class MainWindow(QMainWindow):  # 继承主窗口函数的类, 继承编写的主函数
    ...
    def open_alltable(self):
        ...
        # 连接到数据库并打开
        self.DB = QSqlDatabase.addDatabase('QSQLITE')  # 建立与数据库的连接，即QSqlDatabase对象
        self.DB.setDatabaseName('MES2.db')  # 设置数据库名称
        self.DB.open()  # 打开数据库
        print('数据库打开成功')
        ## 打开现有库存数据表
        self.current_tabModel = QSqlTableModel(self, self.DB)  # 创建数据表的模型
    ...
    def on_enterScan_pushButton_clicked(self):
        ...
        RFID_kind = dict([(value, key) for (key, value) in name_RFID.items()])
        RFID_manufacture = {'E0000001': '西安交大一厂', 'E0000002': '西安交大二厂', 'E0000003': '西安交大三厂'}
        if self.enter_tabModel.record(0).isNull('批次') == True:
            max_batch = 0
        else:
            max_batch = self.enter_tabModel.record(n - 1).value('批次')  # 批次按顺序排列，查找最大批次
        date = self.ui.dealdate.date().toString(Qt.ISODate)
...
from Plotly_Dialog import Ui_Dialog
...
class MainWindow(QMainWindow):  # 继承主窗口函数的类, 继承编写的主函数
    def __init__(self, parent=None):
        ...
        self.outList = {}
        self.capacity = {'直齿轮': 18, '斜齿轮': 18, '齿轮轴': 24, '光轴': 24, '深沟球轴承': 36, '调心球轴承': 36}
        self.nameNum = {'直齿轮': 3, '斜齿轮': 3, '齿轮轴': 4, '光轴': 4, '深沟球轴承': 6, '调心球轴承': 6}
        self.locateName = {'直齿轮': ['A', 'B', 1], '斜齿轮': ['C', 'D', 1], '齿轮轴': ['A', 'B', 2],
                      '光轴': ['C', 'D', 2], '深沟球轴承': ['A', 'B', 3], '调心球轴承': ['C', 'D', 3]}
        self.get_locState()
        # 初始化出入库按钮使能状态
        self.ui.outScan_pushButton.setEnabled(False)
        self.ui.enterScan_pushButton.setEnabled(False)
    ...
    def open_alltable(self):
        ...
        # 连接到数据库并打开
        self.DB = QSqlDatabase.addDatabase('QSQLITE')  # 建立与数据库的连接，即QSqlDatabase对象
        self.DB.setDatabaseName('MES3.db')  # 设置数据库名称
        self.DB.open()  # 打开数据库
        print('数据库打开成功')
        ## 打开现有库存数据表
        self.current_tabModel = QSqlTableModel(self, self.DB)  # 创建数据表的模型
...
#################################路径规划与货位分配###########################################
    ...
    #入库货位分配
    def Enter_allocation(self,name, num):  # 输入工件名称和对应的数量
        itsloc = self.locationState[name]
        for i in range(3):
            if 2 * (i + 1) * self.nameNum[name] - np.sum(itsloc[:, 0:i + 1]) >= num:
                forward_double = i + 1
                break
        #print(forward_double)  # 前forward_double对货架需要存放货物
        idle_loc = np.where(itsloc[:, 0:forward_double] == 0)  # 寻找前forward_double对货架的空闲货位
        locating = []  # 存放接收工件的货位
        for i in range(num):
            x = idle_loc[0][i]
            y = idle_loc[1][i]
            z = idle_loc[2][i]
            self.locationState[name][x][y][z] = 1  #分配好后即更新货位状态
        ...
        for i in locating:
            ...
            EncodeLoc.append(s)
        # print('接收' + name + '的位置列表为：', locating)
        # print('接收后的货位状态为：', self.locationState)
        return EncodeLoc
    #出库工件分配
    def Out_allocation(self,name, num):  # 输入工件名称和对应的数量
        itsloc = self.locationState[name]
        for i in range(3):
            if np.sum(itsloc[:, 0:i + 1]) >= num:
                forward_double = i + 1
                break
        idle_loc = np.where(itsloc[:, 0:forward_double] == 1)  # 寻找前forward_double对货架的空闲货位
        locating = []  # 存放接收工件的货位
        for i in range(num):
            x = idle_loc[0][i]
            y = idle_loc[1][i]
            z = idle_loc[2][i]
            self.locationState[name][x][y][z] = 0  #分配好后即更新货位状态
            locating.append([x, y, z])
        EncodeLoc=[]
        for i in locating:
            s = self.locateName[name][i[0]] + str(i[1] + 1) + '-' + \
    ...
    def on_enterScan_pushButton_clicked(self):
        ...
        RFID_kind = dict([(value, key) for (key, value) in name_RFID.items()])
        all_location=[]
        for key,value in self.enterList.items():
            loation=self.Enter_allocation(key,value)
            all_location=all_location+loation
        RFID_manufacture = {'E0000001': '西安交大一厂', 'E0000002': '西安交大二厂', 'E0000003': '西安交大三厂'}
        if self.enter_tabModel.record(0).isNull('批次') == True:
            max_batch = 0
        else:
...
class MainWindow(QMainWindow):  # 继承主窗口函数的类, 继承编写的主函数
    ...
    def enter(self, enterRFID, all_location):
        ...
        n = self.enter_tabModel.rowCount()  # 库存的已记录数
        RFID_manufacture = {'E0000001': '西安交大一厂', 'E0000002': '西安交大二厂', 'E0000003': '西安交大三厂'}
        if self.enter_tabModel.record(0).isNull('批次') == True:  # 检测入库记录里的最大批次
            max_batch = 0
        else:
            max_batch = self.enter_tabModel.record(n - 1).value('批次')  # 批次按顺序排列，查找最大批次
    ...
    def on_emitPushButton_pressed(self):
        ...
        location, position = self.Enter_allocation(kind, 1)
        ...
        if RFID != []:
            for i in RFID:
                ...
                if kind in inkindNum:
                    ...
                    inkindNum[kind] = 1
            inLocation, forwardLocation = self.getLocation(inkindNum, self.Enter_allocation)
            self.ui.widget.get_path(forwardLocation, 'i')  #绘制路径
    ...
    def recordDemo(self, model):
        ...
        for i in range(enterRow):
            ...
            dateRecord = {}
            if kind not in record:  # 工件种类第一次检索到，将其添加到reord中
                dateRecord[date] = 1  # 同时将对应的日期添加到record中
                record[kind] = dateRecord
            else:
                if date not in record[kind]:  # 种类存在，但是第一次检索到某个日期时
                    record[kind][date] = 1
